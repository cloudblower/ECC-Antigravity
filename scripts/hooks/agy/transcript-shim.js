...
function shimTranscriptPath(agyPath, shadowPath, options = {}) {
  ...
  if (!shadowStat || agyStat.mtimeMs > shadowStat.mtimeMs) {
    ...
    // Atomic write: write to unique temp file then rename to prevent corruption from concurrent hooks
    const tmpPath = `${shadowPath}.${process.pid}.${Date.now()}-${Math.random().toString(36).slice(2)}.tmp`;
...
function shimClaudeTranscriptToAgy(claudePath, agyShadowPath) {
  ...
  if (!shadowStat || claudeStat.mtimeMs > shadowStat.mtimeMs) {
    ...
    // Atomic write: write to unique temp file then rename to prevent corruption from concurrent hooks
    const tmpPath = `${agyShadowPath}.${process.pid}.${Date.now()}-${Math.random().toString(36).slice(2)}.tmp`;
    fs.writeFileSync(tmpPath, shadowLines.join('\n') + '\n', 'utf8');
