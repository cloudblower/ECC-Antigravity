...
| ------------------ | ------------------------------------ | ----------------------------------- | --------------------------------------------------- |
| `//path`           | Absolute path from filesystem root   | `Read(//Users/username/secrets/**)` | `/Users/username/secrets/**`                        |
| `~/path`           | Path from home directory             | `Read(~/Documents/*.pdf)`           | `/Users/username/Documents/*.pdf`                   |
| `/path`            | Path relative to the settings source | `Edit(/src/**/*.ts)`                | `<project root>/src/**/*.ts` in project settings    |
| `path` or `./path` | Path relative to current directory   | `Read(*.env)`                       | `<cwd>/*.env`                                       |

<Warning>
  A pattern like `/Users/username/file` isn't an absolute path. The single leading slash anchors at the settings source, not the filesystem root. Use `//Users/username/file` for absolute paths.
</Warning>
...
On Windows, paths are normalized to POSIX form before matching. `C:\Users\username` becomes `/c/Users/username`, so use `//c/**/.env` to match `.env` files anywhere on that drive. To match across all drives, use `//**/.env`.
