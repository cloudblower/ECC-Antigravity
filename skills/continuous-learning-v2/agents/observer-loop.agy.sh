...
# Include ECC-private storage (~/.ecc/bin) in PATH if present so agy can run
# in environments without a global CLI installation (e.g. Antigravity Desktop).
if [ -d "${HOME}/.ecc/bin" ]; then
  case ":$PATH:" in
    *":${HOME}/.ecc/bin:"*) ;;
    *) export PATH="${HOME}/.ecc/bin:$PATH" ;;
  esac
fi
...
CACHED_OBSERVER_MODEL=""

# Dynamically resolve model from available models in Antigravity CLI without hardcoding slugs.
# Per docs/ANTIGRAVITY-CLI-HEADLESS-MODE.md: `agy models` lists active model slugs.
# In headless mode, --model is optional and pinning an unsupported model fails loudly.
resolve_agy_model() {
  if [ -n "$CACHED_OBSERVER_MODEL" ]; then
    echo "$CACHED_OBSERVER_MODEL"
    return
  fi

  local requested="${ECC_OBSERVER_MODEL:-}"
  local available_models
  available_models=$(agy models 2>/dev/null | sed -e 's/\x1b\[[0-9;]*m//g' | tr -d '\r' | grep -E '^[a-zA-Z0-9]' | awk '{print $1}')

  if [ -z "$available_models" ]; then
    # If agy models is unavailable (e.g. offline/error), pass requested model if specified,
    # or empty so agy uses its default configured model.
    [ -n "$requested" ] && echo "$requested"
    return
  fi
  ...
  case "$requested" in
    haiku|flash|fast|"")
      # Dynamically pick the first available flash model (preferring medium if available)
      resolved=$(echo "$available_models" | grep -E 'flash-medium' | head -n 1)
      if [ -z "$resolved" ]; then
        resolved=$(echo "$available_models" | grep -E 'flash' | head -n 1)
      fi
      ;;
    opus|pro|heavy)
      # Dynamically pick available reasoning/high-capacity model
      resolved=$(echo "$available_models" | grep -E 'opus' | head -n 1)
      if [ -z "$resolved" ]; then
        resolved=$(echo "$available_models" | grep -E 'pro-high|pro' | head -n 1)
      fi
      ;;
    sonnet)
      resolved=$(echo "$available_models" | grep -E 'sonnet' | head -n 1)
      if [ -z "$resolved" ]; then
        resolved="$requested"
      fi
      ;;
    *)
      # Explicit model slug specified by user
      resolved="$requested"
      ;;
  esac

  CACHED_OBSERVER_MODEL="$resolved"
  echo "$resolved"
}
...
analyze_observations() {
  ...
  # Sanitize input: only pass well-formed JSON lines to the analysis
  tail -n "$MAX_ANALYSIS_LINES" "$OBSERVATIONS_FILE" | grep -E '^\{.*\}$' > "$analysis_file" || true
  ...
  # paths like /c/Users/... which native Windows node/python binaries cannot read.
  analysis_relpath=".observer-tmp/$(basename "$analysis_file")"

  prompt_file="$(mktemp "${observer_tmp_dir}/ecc-observer-prompt.XXXXXX")"
  cat > "$prompt_file" <<PROMPT
IMPORTANT: You are running in non-interactive --print mode. You MUST use the write_to_file tool directly to create files. Do NOT ask for permission, do NOT ask for confirmation, do NOT output summaries instead of writing. Just read, analyze, and write.
...
If you find 3+ occurrences of the same pattern, you MUST write an instinct file directly to ${INSTINCTS_DIR}/<id>.yaml using the write_to_file tool.
...
PROMPT
  ...
  if [ -n "${ECC_OBSERVER_MAX_TURNS:-}" ]; then
    ...
    max_turns=$(( analysis_count / ANALYSIS_LINES_PER_TURN ))
    if [ "$max_turns" -lt "$DEFAULT_MAX_TURNS" ]; then max_turns="$DEFAULT_MAX_TURNS"; fi
  ...
  if [ "$max_turns" -lt "$MIN_MAX_TURNS" ]; then
    max_turns="$DEFAULT_MAX_TURNS"
  fi
  ...
  # stdin is explicitly closed with </dev/null.
  # Model is dynamically resolved via `agy models` without hardcoding model slugs.
  observer_model=$(resolve_agy_model)
  model_args=()
  if [ -n "$observer_model" ]; then
    model_args=(--model "$observer_model")
  fi

  # Permissions flag: defaults to --dangerously-skip-permissions for compatibility,
  # but can be scoped via ECC_AGY_OBSERVER_PERMISSIONS.
  observer_permissions="${ECC_AGY_OBSERVER_PERMISSIONS:---dangerously-skip-permissions}"

  ECC_SKIP_OBSERVE=1 ECC_HOOK_PROFILE=minimal agy "${model_args[@]}" $observer_permissions \
  ...
  if [ -f "$OBSERVATIONS_FILE" ]; then
    archive_dir="${ECC_OBSERVER_ARCHIVE_DIR:-${PROJECT_DIR}/observations.archive}"
    mkdir -p "$archive_dir"
