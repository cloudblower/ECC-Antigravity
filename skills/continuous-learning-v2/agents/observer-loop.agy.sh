...
PROMPT
  ...
  # Model is dynamically resolved via `agy models` without hardcoding model slugs (docs/ANTIGRAVITY-CLI-HEADLESS-MODE.md).
  # If ECC_OBSERVER_MODEL is unset, dynamically resolves to an available flash model.
  # If no model is matched or query fails, --model is omitted so agy uses its default model.
  ...
  ECC_SKIP_OBSERVE=1 ECC_HOOK_PROFILE=minimal agy "${model_args[@]}" --dangerously-skip-permissions \
