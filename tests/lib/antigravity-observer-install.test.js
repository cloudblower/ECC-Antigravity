...
test('cleanShellProfiles strips private bin directory from Windows PowerShell profiles', () => {
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecc-agy-clean-ps-profiles-'));
  try {
    const psDir = path.join(tempHome, 'Documents', 'PowerShell');
    fs.mkdirSync(psDir, { recursive: true });
    const psProfile = path.join(psDir, 'Microsoft.PowerShell_profile.ps1');
    fs.writeFileSync(psProfile, '# PowerShell profile\n$env:Path = "C:\\some\\path;" + $env:Path\n$env:Path = "C:\\Users\\User\\.ecc\\bin;" + $env:Path\n');

    cleanShellProfiles(tempHome, 'C:\\Users\\User\\.ecc\\bin');

    const cleaned = fs.readFileSync(psProfile, 'utf8');
    assert.ok(!cleaned.includes('.ecc\\bin'), 'PowerShell profile should have .ecc\\bin removed');
    assert.ok(cleaned.includes('C:\\some\\path'), 'PowerShell profile should retain other PATH entries');
  } finally {
    fs.rmSync(tempHome, { recursive: true, force: true });
  }
});

test('installPrivateAgy executes win32 install command targeting .ecc/bin with flags', () => {
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecc-agy-win-exec-'));
  try {
    const executedCommands = [];
    const mockInstaller = (cmd, opts) => {
      executedCommands.push({ cmd, opts });
      const expectedBinary = path.join(tempHome, '.ecc', 'bin', 'agy.exe');
      fs.writeFileSync(expectedBinary, 'binary content');
      return { status: 0 };
    };
    ...
    assert.strictEqual(executedCommands.length, 1);
    const invokedCmd = executedCommands[0].cmd;
    assert.ok(invokedCmd.includes('irm https://antigravity.google/cli/install.ps1'), 'Command should download Windows installer');
    assert.ok(invokedCmd.includes(path.join(tempHome, '.ecc', 'bin')), 'Command should point to private bin dir');
  } finally {
    fs.rmSync(tempHome, { recursive: true, force: true });
  }
});
