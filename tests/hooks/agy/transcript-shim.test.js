...
test('shimTranscriptPath: correctly recognizes models across full session from synthetic transcript', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agy-synthetic-transcript-test-'));
  const fixturePath = path.join(tmpDir, 'transcript.jsonl');
  const shadowPath = path.join(tmpDir, 'claude-compat-transcript.jsonl');
  ...
  for (let i = 0; i <= 36; i++) {
    if (i === 0) {
      lines.push(JSON.stringify({
        ...
        type: 'USER_INPUT',
        source: 'USER_EXPLICIT',
        content: '<USER_SETTINGS_CHANGE>The user changed setting Model Selection from None to Gemini 3.6 Flash (Low).</USER_SETTINGS_CHANGE>'
      }));
    } else if (i === 5) {
      lines.push(JSON.stringify({
        step_index: 5,
        type: 'USER_INPUT',
        source: 'USER_EXPLICIT',
        content: '<USER_SETTINGS_CHANGE>The user changed setting Model Selection from Gemini 3.6 Flash (Low) to Gemini 3.8 Flash (Medium).</USER_SETTINGS_CHANGE>'
      }));
    } else if (i === 9) {
      lines.push(JSON.stringify({
        step_index: 9,
        type: 'USER_INPUT',
        source: 'USER_EXPLICIT',
        content: '<USER_SETTINGS_CHANGE>The user changed setting Model Selection from Gemini 3.8 Flash (Medium) to Gemini 3.6 Flash (Low).</USER_SETTINGS_CHANGE>'
      }));
    } else if (i === 33) {
      lines.push(JSON.stringify({
        step_index: 33,
  ...
  shimTranscriptPath(fixturePath, shadowPath);
  const shadowLines = fs.readFileSync(shadowPath, 'utf8').trim().split('\n');
...
test('safeAtomicWrite: writes and overwrites file atomically without data corruption', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'safe-atomic-test-'));
  const targetFile = path.join(tmpDir, 'output.txt');

  safeAtomicWrite(targetFile, 'initial content\n');
  assert.strictEqual(fs.readFileSync(targetFile, 'utf8'), 'initial content\n');

  safeAtomicWrite(targetFile, 'updated content\n');
  assert.strictEqual(fs.readFileSync(targetFile, 'utf8'), 'updated content\n');
