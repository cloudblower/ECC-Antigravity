...
test('pre-tool-use exits 0 on invalid JSON stdin (fail-open)', () => {
  const result = spawnSync('node', [path.join(__dirname, '../../../scripts/hooks/agy/pre-tool-use.js')], {
    input: 'this is not json'
  });
  assert.strictEqual(result.status, 0, 'Shim must exit 0 on stdin JSON parse failure');
});

test('pre-tool-use handles missing or undefined toolCall safely without throwing', () => {
  const payload = {
    conversationId: 'test-no-tool-call',
    workspacePaths: [testDir]
  };
  const result = spawnSync('node', [path.join(__dirname, '../../../scripts/hooks/agy/pre-tool-use.js')], {
    input: JSON.stringify(payload)
  });
  assert.strictEqual(result.status, 0, 'Shim should exit 0 when toolCall is absent');
  const output = JSON.parse(result.stdout.toString());
  assert.strictEqual(output.decision, 'allow');
});
