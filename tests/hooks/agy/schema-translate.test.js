'use strict';
/**
 * Tests for scripts/hooks/agy/schema-translate.js
 */
...
test('mapToolInputToAgy: maps Edit tool_input to replace_file_content args with schema defaults', () => {
  const claudeInput = { file_path: '/path/to/file.js', old_string: 'foo', new_string: 'bar' };
  const agyArgs = mapToolInputToAgy('Edit', claudeInput);
  assert.deepStrictEqual(agyArgs, {
...
test('mapToolInputToAgy: maps Edit tool_input with replace_all to AllowMultiple: true', () => {
  const claudeInput = { file_path: '/path/to/file.js', old_string: 'foo', new_string: 'bar', replace_all: true };
  const agyArgs = mapToolInputToAgy('Edit', claudeInput);
  assert.strictEqual(agyArgs.AllowMultiple, true);
});
...
test('mapToolNameToClaude: handles stringified agyArgs for call_mcp_tool', () => {
  const agyArgsString = JSON.stringify({ ServerName: 'context7', ToolName: 'fetch_docs' });
  assert.strictEqual(mapToolNameToClaude('call_mcp_tool', agyArgsString), 'mcp__context7__fetch_docs');
});
...
test('mapToolInputToClaude and mapToolInputToAgy handle null and undefined safely', () => {
  assert.deepStrictEqual(mapToolInputToClaude('run_command', null), {});
  assert.deepStrictEqual(mapToolInputToClaude('write_to_file', undefined), {});
  assert.deepStrictEqual(mapToolInputToClaude('unknown_tool', null), {});

  assert.deepStrictEqual(mapToolInputToAgy('Bash', null), {});
  assert.deepStrictEqual(mapToolInputToAgy('Write', undefined), {});
  assert.deepStrictEqual(mapToolInputToAgy('Unknown', null), {});
});
